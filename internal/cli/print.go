...
func printFunctionInfo(fn agent.FunctionConfig) {
...
func printAgentInfo(agentCfg agent.Agent, agentName string) {
	...
	if agentCfg.Name != "" {
		fmt.Printf("  %s (%s): ", nameStyle(agentCfg.Name), agentNameStyle(agentName))
	} else {
	...
	if agentCfg.Description != "" {
		fmt.Println(agentCfg.Description)
	} else {
...
func printDetailedAgentInfo(agentCfg agent.Agent, agentPath string) {
	labelStyle := color.New(color.FgHiCyan, color.Bold).SprintFunc()

	// Print agent name and path
	if agentCfg.Name != "" {
		fmt.Printf("  %s %s\n", labelStyle("Name:"), agentCfg.Name)
	}
	if agentCfg.Description != "" {
	...
	if agentCfg.DefaultModel != "" {
		fmt.Printf("  %s %s\n", labelStyle("Default Model:"), agentCfg.DefaultModel)
	}

	fmt.Printf("  %s %d\n", labelStyle("Functions:"), len(agentCfg.Functions))
	fmt.Printf("  %s %d\n", labelStyle("MCP Servers:"), len(agentCfg.MCPServers))
}
...
func printOutput(history history.ConversationHistory, pretty bool) {
	...
	if pretty {
		conversation.PrintPrettyOutput(lastMessage.Content)
	} else {
...
func printMCPServerInfo(name string, server agent.MCPServerConfig) {
	...
	client := mcp.NewMCPClient(name, server)
