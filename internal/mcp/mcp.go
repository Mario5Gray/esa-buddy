...
func NewMCPClient(name string, config agent.MCPServerConfig) *MCPClient {
	return &MCPClient{
		name:           name,
		config:         config,
...
func (c *MCPClient) CallTool(toolName string, arguments interface{}, askLevel string) (string, error) {
	...
	if tools.NeedsConfirmation(askLevel, isSafe) {
		...
		response := utils.Confirm(fmt.Sprintf("Call %s:%s(%s)?", c.name, actualToolName, argsDisplay))
...
func (m *MCPManager) StartServers(ctx context.Context, mcpServers map[string]agent.MCPServerConfig) error {
	m.mu.Lock()
	defer m.mu.Unlock()
