...
func validateAgent(agent Agent) (Agent, error) {
	...
	for i, fc := range agent.Functions {
		...
		agent.Functions[i].Description, err = utils.ProcessShellBlocks(fc.Description)
		if err != nil {
		...
		for j, param := range fc.Parameters {
			...
			agent.Functions[i].Parameters[j].Description, err = utils.ProcessShellBlocks(param.Description)
			if err != nil {
