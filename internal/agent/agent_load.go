...
func Load(agentPath string) (Agent, error) {
...
func LoadConfiguration(opts *options.CLIOptions) (Agent, error) {
	...
	agentPath := utils.ExpandHomePath(opts.AgentPath)
...
func loadAgentFromFile(agentPath string, visited map[string]bool, depth int) (Agent, error) {
	...
	resolvedPath := utils.ExpandHomePath(agentPath)
...
func loadAgentFromRef(ref string, childPath string, visited map[string]bool, depth int) (Agent, error) {
	...
	userAgentPath := utils.ExpandHomePath(fmt.Sprintf("%s/%s.toml", AgentsDir(), ref))
